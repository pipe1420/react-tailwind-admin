...
const AppHeader: React.FC = () => {
  ...
  // 👇 NUEVOS ESTADOS: Controlar qué menú está abierto a nivel de Header
      ...
      <div className="flex flex-col items-center justify-between grow lg:flex-row lg:px-6">
        <div className="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4">
          ...
          <Link to="/" className="lg:hidden">
            <img className="dark:hidden" src="./images/logo/logo_edificios.svg" alt="Logo" width={40} height={40} />
            <img className="hidden dark:block" src="./images/logo/logo_edificios.svg" alt="Logo" width={40} height={40} />
          </Link>
        ...
        <div className={`${isApplicationMenuOpen ? "flex" : "hidden"} items-center justify-between w-full gap-4 px-5 py-4 lg:flex shadow-theme-md lg:justify-end lg:px-0 lg:shadow-none`}>
          <div className="flex items-center gap-2 2xsm:gap-3">
            ...
            {/* 👇 PASAR CONTROL DE ESTADO AL DROPDOWN DE NOTIFICACIONES */}
            <NotificationDropdown 
              isOpen={activeDropdown === "notifications"}
              setOpen={(open) => setActiveDropdown(open ? "notifications" : null)}
            />
          </div>
          
          {/* 👇 PASAR CONTROL DE ESTADO AL DROPDOWN DE USUARIO */}
          <UserDropdown 
            isOpen={activeDropdown === "user"}
            setOpen={(open) => setActiveDropdown(open ? "user" : null)}
          />
        </div>
      </div>
    </header>
